...
-	**High availability and resiliency of virtualized workloads:** To meet existing workload availability service level agreements the OpenShift Virtualization clusters should be sized to tolerate node failures. Features such as Node health check and fence agent remediation operators can detect node failures or hung nodes. To tolorate zone level outages, you can use soft eviction thresholds set below the service level agreement defined, to trigger live migrations and avoid workload downtime.
